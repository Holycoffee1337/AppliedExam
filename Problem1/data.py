...
class DataLoader():
    def __init__(self, path='C:/Users/chris/OneDrive/Dokumenter/SDU/9semester/DS807-Anvendt_maskinlæring/Project/Main/Problem1/Data/patch_camelyon/'):
        self.path = path
    ...
    def load_data(self):
        ds1, ds2, ds3 = tfds.load('patch_camelyon', split=['train[:1%]', 'test[:1%]', 'validation[:1%]'],
                                  data_dir=self.path,
                                  download=False,
                                  shuffle_files=True)
        return ds1, ds2, ds3
    def plot_raw(self):
        x,y=tfds.load('patch_camelyon', split='train',with_info=True,
                                  data_dir=self.path,
                                  download=False,
                                  shuffle_files=True)
        tfds.show_examples(x,y)

    def plot_current(self, batch):
        plt.figure(figsize=(10, 10))
        for i, sample in enumerate(batch.take(9)):  # Take 1 batch for visualization
            images, labels = sample[0], sample[1]  # Adjust the indices based on your dataset structure
            plt.subplot(3, 3, i + 1)
